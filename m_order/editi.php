...
include("../../config.php");   
$response = [];
ob_clean(); // Clear any output from config.php or includes
...
if (isset($_POST['id'])) {
    ...
    $query->bindParam(':ttd_id', $_POST['ttd_id']);
    $query->bindParam(':cap_id', $_POST['cap_id']);
    ...
    try {
        if ($query->execute()) {
            ...
            $response['errorInfo'] = $query->errorInfo();
        }
    } catch (Exception $e) {
        $response['code'] = 500;
