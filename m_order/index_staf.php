...
if( $_SESSION['level_id'] == "2" || $_SESSION['level_id'] == "5" || $_SESSION['level_id'] == "6"){
}else{
...
while($data = $sql->fetch()) {
    ...
    $status = $statusList[$data['status']] ?? 'Tidak Diketahui';

    // order_kat
    $stmt = $conn->prepare("SELECT nama FROM m_order_kat WHERE id = :id");
    $stmt->execute([':id'=>$data['order_kat_id']]);
    $kat = $stmt->fetch()['nama'];

    // order_katsub
    $stmt = $conn->prepare("SELECT nama FROM m_order_katsub WHERE id = :id");
    $stmt->execute([':id'=>$data['order_katsub_id']]);
    $subkat = $stmt->fetch()['nama'];
?>
<tr>
    <td><?= $count++; ?></td>
    ...
    <!-- ORDER COLUMN -->
    <td>
        <div><b><?= $kat; ?> - <?= $subkat ?></b></div>
        <div><small><?= date("d-m-Y H:i:s", strtotime($data['created_at'])) ?></small></div>
        <div><small>Nama: <?= $data['nama']; ?></small></div>

        <div class="mt-5">
            <span class="label bg-green btn_update"
                  data-id="<?= $data['id'] ?>" 
    ...
    <td>
        ...
        if ($data['shipper_id'] == 0) {
            ...
            echo htmlspecialchars($stmt->fetch()['nama']);
        }
        ?>
        <br>
        <!--<a data-toggle="modal" data-target="#shipper" -->
        <!--   data-id="<?= $data['id'] ?>"-->
        <!--   style="cursor:pointer;">Ubah</a>-->
    </td>
    ...
    <td>
        ...
        ?>
        <br>
        <!--<a data-toggle="modal" data-target="#consignee" -->
        <!--   data-id="<?= $data['id'] ?>"-->
        <!--   style="cursor:pointer;">Ubah</a>-->
    </td>
                        ...
                        <select id="rek_select" class="form-control">
                            ...
                            <?php foreach ($caps as $cap): ?>
                                <option value="<?php echo ($cap['id']); ?>">
                                    <?php echo htmlspecialchars($cap['nama']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
...
<script type="text/javascript">
     $(document).ready(function(){
        ...
        $(document).on('click','.btn_invoice',function(){
            ...
            // Set value untuk select2 dan hidden input
            ...
            // Trigger change untuk select2 agar tampilan terupdate
 ...
 $(function(){
  ...
  // Inisialisasi Select2 untuk Tanda Tangan dan Cap di modal invoice
  $('#ttd_select').select2({
