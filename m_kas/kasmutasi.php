...
if (isset($_GET['filter']) && $_GET['filter'] == 'filter' && !empty($_GET['tanggal']) && !empty($_GET['sd'])) {
    ...
    // Query pertama: hitung total
    $sqlHitung = $conn->prepare("
        SELECT * FROM m_kas 
        WHERE stat IN (1, 2, 4)
    ...
    if (date("Y-m-d", strtotime($data['created_at'])) == date("Y-m-d")) {
        $pengeluaranHariIni += $nilai;
    }

    // Total semua nilai (termasuk piutang)
    $totals += $nilai;
    }

    // Query kedua: untuk tampilan
    $sql = $conn->prepare("
        SELECT * FROM m_kas 
        WHERE stat IN (1, 2, 4)
        AND created_at BETWEEN :tanggal_awal AND :tanggal_akhir 
...
<!-- ✅ TOTAL DITAMPILKAN DI ATAS -->
<div class="row mb-3">
    <div class="col-md-12">
        <div class="alert " style="background-color: #ffffff; color: #000;">
            <strong>Total Masuk:</strong> Rp. <?= number_format($totalMasuk, 0) ?>,-<br>
            <strong>Total Keluar:</strong> Rp. <?= number_format($totalKeluar, 0) ?>,-<br>
            <strong>Keuntungan Bersih:</strong> Rp. <?= number_format($totalMasuk - $totalKeluar, 0) ?>,-<br> 
            <br>
            <strong>Total Piutang:</strong> Rp. <?= number_format($totalPiutang, 0) ?>,-<br>
        </div>
    </div>
</div>
