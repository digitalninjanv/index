...
// Mengambil parameter codx dari URL
$codx = isset($_GET['codx']) ? $_GET['codx'] : '';

// Validasi jika codx kosong
if ($codx == '') {
    header('Location: piutangs.php');
    exit;
}

// Query mengambil data order berdasarkan CODX
$orderStmt = $conn->prepare("SELECT mo.*, u.nama AS shipper_nama FROM m_order mo LEFT JOIN m_user u ON u.id = mo.shipper_id WHERE mo.codx = ?");
$orderStmt->execute([$codx]);
$orderData = $orderStmt->fetch();

// Cek apakah data order ditemukan
if (!$orderData) {
    echo "<script>alert('Data Order dengan Kode $codx tidak ditemukan!'); window.location='piutangs.php';</script>";
    exit;
}

// Ambil ID order untuk query ke tabel m_kas (tetap pakai ID untuk relasi ke m_kas agar performa cepat)
$orderId = $orderData['id'];
// --- PERUBAHAN SELESAI DISINI ---

// Query Total Piutang (tetap menggunakan order_id yang didapat dari codx diatas)
  ...
  <div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Detail Order <?php echo $orderData['codx']; ?></li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <div class="box-body">
              <div class="row">
                <div class="col-md-6">
                  <p><strong>Order ID (System):</strong> <?php echo $orderId; ?></p> 
